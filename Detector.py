...
#             G = min(G, C)
#
#             x = tf.split(axis= -1,num_or_size_splits=G,value= tf.expand_dims(x,-2))
#             x = tf.concat(x,-2)
...
#
#             gamma = tf.get_variable(name='gamma', shape=[1, 1, 1, C], initializer=tf.constant_initializer(1.0))
#             beta = tf.get_variable(name='beta', shape=[1, 1, 1, C], initializer=tf.constant_initializer(0.0))
#
#             x = tf.split(axis=-2, num_or_size_splits=G, value=x)
#             x = tf.concat(x,-1)
#             x = tf.squeeze(x,-2)
#
#             x = x * gamma + beta
...
#                                    trainable=trainable)
#                 if conv['gnorm']:
#                     x=self._group_norm(x,layer_idx=conv['layer_idx'])
#                 if conv['bnorm']:
#                     x=self._batch_norm(x, conv)
#
#                 if conv['leaky']:
...
#                      'layer_idx': 16 + i * 3},
#                     {'filter': 256, 'kernel': 3, 'stride': 1, 'bnorm': True, 'gnorm': False, 'leaky': True,
#                      'layer_idx': 17 + i * 3}],isTrain = isTrain)
#             skip_36 = x
#             # Layer 37 => 40
#             x = self._conv_block(x, [
...
#                      'layer_idx': 41 + i * 3},
#                     {'filter': 512, 'kernel': 3, 'stride': 1, 'bnorm': True, 'gnorm': False, 'leaky': True,
#                      'layer_idx': 42 + i * 3}],isTrain = isTrain)
#             skip_61 = x
#             # Layer 62 => 65
#             x = self._conv_block(x, [
...
#                      'layer_idx': 66 + i * 3},
#                     {'filter': 1024, 'kernel': 3, 'stride': 1, 'bnorm': True, 'gnorm': False, 'leaky': True,
#                      'layer_idx': 67 + i * 3}],isTrain = isTrain)
#             # Layer 75 => 79
#             x = self._conv_block(x, [
#                 {'filter': 512, 'kernel': 1, 'stride': 1, 'bnorm': True, 'gnorm': False, 'leaky': True,
#                  'layer_idx': 75},
...
#                 train_batch = self.trainBatchGenerator.__getitem__(iter)
#
...
class ExperimentDetector():
    def __init__(self,sess= tf.InteractiveSession(),isRestore=False):
        ...
        self.logits = self.make_yolov3_model(self.input_img,isTrain=self.isTrain)



        self.loss = tf.sqrt(tf.reduce_sum(self.LossFunction(self.input_img, self.logits, self.ground_truth, self.ground_truth_boxes)))
        #self.sqrt_loss = tf.sqrt(self.loss)
    ...
    def _group_norm(self,x,layer_idx, G=16, eps=1e-5, scope='gnorm'):
        ...
        return x
    def _batch_norm(self,x,conv, isTrain=False,  eps=1e-5, scope='bnorm'):
        with tf.variable_scope(scope+'_'+str(conv['layer_idx'])):
            x=tf.layers.batch_normalization(inputs=x,training=isTrain)
        return x
    def _conv_block(self,inp, convs, isTrain=False, skip=True,trainable=True):
        with tf.variable_scope('conv_block'):
            x = inp
            count = 0
            for conv in convs:
                ...
                if conv['gnorm']:
                    x=self._group_norm(x,layer_idx=conv['layer_idx'])
                if conv['bnorm']:
                    x=self._batch_norm(x, conv,isTrain = isTrain)

                if conv['leaky']:
                    x = tf.nn.leaky_relu(x,alpha=0.1, name='leaky_' + str(conv['layer_idx']))
        return skip_connection+x if skip else x
    def make_yolov3_model(self,input_image,isTrain=False,reuse=False):
        with tf.variable_scope('yolo3',reuse=reuse):
            # Layer  0 => 4
            x = self._conv_block(input_image,
                                 [{'filter': 32, 'kernel': 3, 'stride': 1, 'bnorm': True, 'gnorm': False, 'leaky': True,
            ...
            x = self._conv_block(x, [
                ...
                {'filter': 512, 'kernel': 1, 'stride': 1, 'bnorm': True, 'gnorm': False, 'leaky': True,
                 'layer_idx': 79}],
                                 skip=False,isTrain = isTrain)
        with tf.variable_scope('new_network', reuse=reuse):
            # Layer 80 => 82
            yolo_82 = self._conv_block(x, [
                {'filter': 1024, 'kernel': 3, 'stride': 1, 'bnorm': True, 'gnorm': False, 'leaky': True,
                 'layer_idx': 80},
                {'filter': 3 * (len(self.labels) + 5), 'kernel': 1, 'stride': 1, 'bnorm': False, 'gnorm': False,'leaky': False,
                 'layer_idx': 81}], skip=False,isTrain = isTrain)
        with tf.variable_scope('yolo3', reuse=reuse):
            # Layer 83 => 86
            x = self._conv_block(x, [
                {'filter': 256, 'kernel': 1, 'stride': 1, 'bnorm': True, 'gnorm': False, 'leaky': True,
    ...
    def LossFunction(self, input_image, y_preds, y_trues, true_boxes):
        ...
        for y_idx,y_pred in enumerate(y_preds):
            ...
            recall50 = tf.reduce_sum(tf.to_float(iou_scores >= 0.5)) / (count + 1e-3)
            recall75 = tf.reduce_sum(tf.to_float(iou_scores >= 0.75)) / (count + 1e-3)
            #print('iou_scores',self.sess.run(iou_scores ))
            avg_iou = tf.reduce_sum(iou_scores) / (count + 1e-3)
            avg_obj = tf.reduce_sum(object_mask * pred_box_conf * true_box_conf) / (count + 1e-3)
            avg_noobj = tf.reduce_sum(no_object_mask * pred_box_conf) / (count_noobj + 1e-3)
            avg_cat = tf.reduce_sum(object_mask * pred_box_class * true_box_class) / (count + 1e-3)
            ...
            # loss = tf.Print(loss, [grid_h, net_factor], message='net_factor \t\t', summarize=1000)
            # loss = tf.Print(loss, [grid_h, pred_box_wh], message='pred_box_wh \t\t', summarize=1000)
            # loss = tf.Print(loss, [grid_h, y_pred], message='y_pred \t\t', summarize=1000)
            # #loss = tf.Print(loss, [grid_h, y_trues], message='y_trues \t\t', summarize=1000)
            # loss = tf.Print(loss, [grid_h, avg_obj], message='avg_obj \t\t', summarize=1000)
            # loss = tf.Print(loss, [grid_h, avg_noobj], message='avg_noobj \t\t', summarize=1000)
            # loss = tf.Print(loss, [grid_h, avg_iou], message='avg_iou \t\t', summarize=1000)
            # loss = tf.Print(loss, [grid_h, avg_cat], message='avg_cat \t\t', summarize=1000)
            ...
            # loss = tf.Print(loss, [grid_h, recall75], message='recall75 \t', summarize=1000)
            # loss = tf.Print(loss, [grid_h, count], message='count \t', summarize=1000)
            # loss = tf.Print(loss, [grid_h, loss], message='loss: \t', summarize=1000)
            result_loss.append(tf.reduce_sum(loss))
